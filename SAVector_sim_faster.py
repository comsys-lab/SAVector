...
if __name__ == '__main__':
    ...
    Results_dir = './Eval2_rev'
    input_SRAM = 2097152+16384 # Total input SRAM of an accelerator, unit=Bytes
    weight_SRAM = 2097152+16384 # Total weight SRAM of an accelerator, unit=Bytes
    output_SRAM = 2097152+16384 # Total output SRAM of an accelerator, unit=Bytes
    ##################################

    # DNN model related (Set mnk_flag and topology file)
    if ('BERT' in toponame) or ('GPT' in toponame) or ('TF' in toponame):
        mnk_flag = True 
    else:
        mnk_flag = False 
    ...
    for gds in x_labels:
        ...
        repname = "SAVector_" + SAdim + "SA_" + str(gds) + 'p' +'_B' + str(batch_sz) # e.g., SAVector_32x32SA_4x4p+B16
        repdir=Results_dir + '/SAVector_Detail/'+toponame #./reports_detail/simname_toponame
        createDirectory(Results_dir + '/SAVector_Detail')
        ...
        repdir2=Results_dir+'/SAVector_Compact/'+toponame #./reports_compact/simname_toponame
        createDirectory(Results_dir+'/SAVector_Compact')
