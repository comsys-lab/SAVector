...
class scaled_out_simulator:
    ...
    def gather_stats(self, memory_system_obj, compute_system_obj,row_id, col_id, layer_id):
        ...
        # When fetching a vector of input or weight matrix, max(MAC cycles, BUF latency) is a critical path.
        # When fetching is done (and SA accumulates the partial sums inside the array), BUF latency does not affects the performance.
        # If you want to assume the weight double-buffering within the SA, modify "2*SA_row" to "SA_row".
        # fold_rt = max(self.cycles_for_MAC, self.cycles_for_BUF)*(2*SA_row + this_m) + self.cycles_for_MAC*(SA_col - 1)
        # For SOSA tiling & scheduling
        fold_rt = (SA_row + this_m + SA_col - 1) * ((self.cycles_for_BUF - 1 + self.cycles_for_MAC) / SA_col)
...
if __name__ == '__main__':
    ...
    Cycles_for_MAC = int(SAcol) # Cycles per MAC operation, default=1
    Cycles_for_BUF = int(2 + np.sqrt(num_pods)*4)+1 # Shared buffer access latency
    ##################################
    ...
    for gds in x_labels:
        ...
        repname = SAdim + "SA_" + str(gds) + 'p' +'_batch' + str(batch_sz) # e.g., SOSA_RT_32x32SA_4x4p+B16
        repdir=Results_dir + '/SOSA_RT_Detail/'+toponame #./reports_detail/simname_toponame
        createDirectory(Results_dir + '/SOSA_RT_Detail')
        ...
        repdir2=Results_dir+'/SOSA_RT_Compact/'+toponame #./reports_compact/simname_toponame
        createDirectory(Results_dir+'/SOSA_RT_Compact')
