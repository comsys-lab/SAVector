...
from scalesim.compute.systolic_compute_ws_shared import systolic_compute_ws
from scalesim.compute.systolic_compute_is import systolic_compute_is
from scalesim.memory.double_buffered_scratchpad_mem_shared import double_buffered_scratchpad as mem_dbsp
...
def mkcfg(config_base, dataflow, SArow, SAcol, IF_SRAM, Fil_SRAM, OF_SRAM):
    ...
    cfgfname = SArow + 'x' + SAcol + '_sharedbuf'
    cfgname = config_base + cfgfname +'.cfg'
    Arr_H = int(SArow)
    Arr_W = int(SAcol)
    IF_SRAM = int(input_SRAM) 
    Fil_SRAM = int(weight_SRAM) 
    OF_SRAM = int(output_SRAM)
...
class scaled_out_simulator:
    def __init__(self):
        ...
        # For new runtime analytical model
        self.cycles_for_MAC = 1
        self.cycles_for_BUF = 1

        # Flags
        self.params_valid = False
        self.all_grids_done = False
        self.metrics_ready = False
        # Flags and variables for shared buffer model
        self.get_dram_access = False
        self.first_pod = False
        self.input_concat_start = False
        self.weight_concat_start = False
        self.do_input_concat = False
        self.do_weight_concat = False
        self.fold_rt = 0
        self.critical_rt = 0
    ...
    def set_params(self,
                    ...
                    mnk_input= False,
                    batch_size=1,
                    cycles_for_MAC=1, cycles_for_BUF=1
                    ):
        ...
        self.n_dim = 0
        self.k_dim = 0
        self.cycles_for_MAC = cycles_for_MAC
        self.cycles_for_BUF = cycles_for_BUF
        #
    ...
    #
    def init_shared_buf_flags(self):
        # Flags and variables for shared buffer model
        self.get_dram_access = False
        self.first_pod = False
        self.input_concat_start = False
    ...
    def run_simulation_single_layer(self, layer_id=0):
        ...
        #initialize concatenated demand matrices
        self.init_shared_buf_flags()
        ...
        Used_row = []
        for i in range(self.grid_rows): #pod rows를 하나씩 늘려가면서 테스트.
        ...
        for grid_row_id in range(self.grid_rows):
            for grid_col_id in range(self.grid_cols):
                ...
                # Skip inactivated pods
                if (grid_row_id >= Used_row) | (grid_col_id >= Used_col):
                    continue
                
                # Setting the flags to get accurate dram access count
                if (grid_row_id == (Used_row-1)) and (grid_col_id == (Used_col-1)): # Last pod of pod dimension
                    self.get_dram_access = True
                else:
                    self.get_dram_access = False
                    
                if (grid_row_id == 0) and (grid_col_id == 0):
                    self.first_pod = True
                else:
                    self.first_pod = False
                    
                # Input demand mat concatenation
                if (grid_col_id == (Used_col-1)):
                    self.do_input_concat = True
                else:
                    self.do_input_concat = False
                    
                if (grid_row_id == (Used_row-1)):
                    self.do_weight_concat = True
                else:
                    self.do_weight_concat = False
                ...
                # Setting flags (shared buffer)
                compute_system.set_shbuf(self.first_pod, self.input_concat_start, self.weight_concat_start, self.do_input_concat, self.do_weight_concat, self.fold_rt,\
                    self.critical_rt, self.input_demand_matrix_concat, self.weight_demand_matrix_concat)
                ...
                if self.first_pod:
                    self.fold_rt, self.critical_rt = compute_system.get_runtimes()
                if self.do_input_concat:
                    self.input_demand_matrix_concat, self.input_concat_start = compute_system.get_input_concat()
                if self.do_weight_concat:
                    self.weight_demand_matrix_concat, self.weight_concat_start = compute_system.get_weight_concat()
                ...
                if self.get_dram_access:
                    # memory_system.service_memory_requests(ifmap_demand_mat, filter_demand_mat, ofmap_demand_mat)
                    memory_system.service_memory_requests(self.input_demand_matrix_concat, self.weight_demand_matrix_concat, ofmap_demand_mat)
    ...
    def gather_stats(self, memory_system_obj, compute_system_obj,row_id, col_id, layer_id):
        ...
        # 1. Compute cycles
        # self.stats_compute_cycles[layer_id, indx] = memory_system_obj.get_total_compute_cycles() # old
        ...
        fold_rt = (SA_row + this_m + SA_col - 1)
        fold_rt = np.ceil(fold_rt)
    ...
    def get_report_items_per_pod(self):
        return self.stats_ifmap_dram_reads, self.stats_filter_dram_reads, 0, self.topo_obj.get_num_layers()
...
if __name__ == '__main__':
    ...
    PODdim=args.PODdim
    g_row, g_col = map(int, PODdim.split('x'))
    num_pods=g_row*g_col
    ...
    config_base = './configs/Shared_SOSA/' # Config file folder
    Results_dir = './Eval_rev_batch'
    input_SRAM = 30*1024*1024 # Total input SRAM of a pod, unit=Bytes
    weight_SRAM = 10*1024*1024 # Total weight SRAM of a pod, unit=Bytes
    output_SRAM = 10*1024*1024 # Total output SRAM of an accelerator, unit=Bytes
    Cycles_for_MAC = 1 # Cycles per MAC operation, default=1
    Cycles_for_BUF = 1 # Shared buffer access latency
    ##################################
    ...
    for gds in x_labels:
        ...
        repdir = Results_dir + '/Ideal_Detail/'+toponame #./reports_detail/simname_toponame
        createDirectory(Results_dir + '/Ideal_Detail')
        ...
        grid1.set_params(topology_filename=topofile,
                        ...
                        cycles_for_MAC=Cycles_for_MAC, cycles_for_BUF=Cycles_for_BUF,)
        ...
        ofmap_writes = Get_OFMAP_offchip_writes(toponame, batch_sz)
        ...
        repdir2=Results_dir + '/Ideal_Compact/'+toponame #./reports_compact/simname_toponame
        createDirectory(Results_dir+'/Ideal_Compact')
        ...
        repdir3=Results_dir+'/Ideal_perPod/'+toponame #./reports_compact/simname_toponame
        createDirectory(Results_dir+'/Ideal_perPod')
