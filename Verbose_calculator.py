...
if ("BERT" in toponame) or ('ViT' in toponame):
    mnk_flag = True
else:
    mnk_flag = False

MACs_row = 128 # number of total MAC units
MACs_col = 128
...
#result_this_row = [SA_row, RT, Comp_util, Pod_util, P_Throughput, E_Throughput, MAC_power, SRAM_power, Offmem_power, TOTALpower, P_TPW, E_TPW, MAC_E, SRAM_E, Offmem_E, Total_E]
rep_writer.writerow(["SA dim", "Runtime", "Compute util", "Pod util","P. Throughput (GFLOPS/S)", "E. Throughput (GFLOPS/S)",\
...
for rows in rep_reader:
    ...
    # MAC_E = (E_MAC * MACs_row * MACs_col) * (Pod_util * 0.01) * (RT * (Comp_util * 0.01)) # cycle마다의 MAC power * 실제 연산 수행한 cycles
    # MAC_E = (E_MAC * MACs_row * MACs_col) * (Pod_util * 0.01) * (RT * (Comp_util * 0.01)) + ((E_MAC_static * MACs_row * MACs_col) * (Pod_util * 0.01) * RT) 
    MAC_E = (E_MAC * MACs_row * MACs_col) * (Pod_util * 0.01) * (RT * (PE_ratio * 0.01)) + ((E_MAC_static * MACs_row * MACs_col) * (Pod_util * 0.01) * RT)
    ...
    P_Throughput = (2 * MACs_row * MACs_col) # GFLOPS
    E_Throughput = P_Throughput * (Comp_util * 0.01) # GFLOPS
    MAC_power = ((E_MAC * MACs_row * MACs_col * (PE_ratio * 0.01)) + (E_MAC_static * MACs_row * MACs_col)) * (Pod_util * 0.01) / np.power(10,12) * Clock_freq * np.power(10,3)# mW
    if SA_row >= 128:
        SRAM_power = ((SRAM * (E_BUF)) / RT) / np.power(10,12) * Clock_freq * np.power(10,3)# mW
    else:
        SRAM_power = ((SRAM * (E_BUF + E_tinyBUF)) / RT) / np.power(10,12) * Clock_freq * np.power(10,3)# mW
        # SRAM_power = ((SRAM * (E_BUF)) / RT) / np.power(10,12) * Clock_freq * np.power(10,3)# mW
    # SRAM_power = (( (SRAM+SRAM_R) * (E_BUF)) / RT) / np.power(10,12) * Clock_freq * np.power(10,3)# mW Ideal BUF architecture
    Offmem_power = ((Offmem * E_offmem) / RT) / np.power(10,12) * Clock_freq * np.power(10,3)# mW
    TOTALpower = MAC_power + SRAM_power + Offmem_power # mW
    P_TPW = P_Throughput / TOTALpower
    E_TPW = E_Throughput / TOTALpower
    ...
    if "BERT_base" in toponame:
        ...
        RT = RT * 24
    elif "ViT_huge" in toponame:
        MAC_E = np.round(MAC_E * 36,2)
        SRAM_E = np.round(SRAM_E * 36,2)
        Offmem_E = np.round(Offmem_E * 36,2)
        Total_E = np.round(Total_E * 36,2)
        RT = RT * 36
    ...
    result_this_row = [SA_row, RT, Comp_util, Pod_util, P_Throughput, E_Throughput, MAC_power, SRAM_power, Offmem_power, TOTALpower, P_TPW, E_TPW, MAC_E, SRAM_E, Offmem_E, Total_E, Offmem]
