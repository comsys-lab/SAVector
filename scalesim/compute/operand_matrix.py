...
class operand_matrix(object):
    def __init__(self):
        ...
        self.ifmap_addr_matrix = np.ones((self.ofmap_px_per_filt * self.batch_size, self.conv_window_size), dtype=np.int)
        self.filter_addr_matrix = np.ones((self.conv_window_size, self.num_filters), dtype=np.int)
        self.ofmap_addr_matrix = np.ones((self.ofmap_px_per_filt * self.batch_size, self.num_filters), dtype=np.int)
    ...
    def set_params(self,
        ...
        self.ofmap_addr_matrix = np.ones((self.ofmap_px_per_filt * self.batch_size, self.num_filters), dtype='>i4')
        self.params_set_flag = True

        # DEBUG
        # print('batch_size={}, ifmap_addr_matrix={}'.format(self.batch_size, self.ifmap_addr_matrix.shape))
    ...
    def create_ifmap_matrix(self):
        ...
        #DEBUG
        # print('ifmap_addr_matrix shape (after create_ifmap_matrix): {}'.format(self.ifmap_addr_matrix.shape))
    ...
    def calc_ifmap_elem_addr(self, i, j):
        ...
        # For batch
        num_input_elem = self.ifmap_rows * self.ifmap_cols * self.num_input_channels
        batch_idx = math.floor(i / self.ofmap_px_per_filt)
        # print(batch_idx)
        i = i % self.ofmap_px_per_filt
        # print(i)
        ...
        if c_row + i_row >= self.ifmap_rows or c_col + i_col >= self.ifmap_cols:  # for padded address
            ...
            ifmap_px_addr = internal_address + window_addr + offset + ( batch_idx * (num_input_elem + 100000) ) # Global address
    ...
    def create_ofmap_matrix(self):
        ...
        for row_idx in range(self.ofmap_px_per_filt * self.batch_size):
    ...
    def get_ifmap_matrix_part(self, start_row=0, num_rows=-1, start_col=0,
        ...
        if num_rows == -1:
            ...
            num_rows = self.ofmap_px_per_filt * self.batch_size
        if num_cols == -1:
        ...
        if (start_row + num_rows) > self.ofmap_px_per_filt * self.batch_size or (start_col + num_cols) > self.conv_window_size:
    ...
    def get_ofmap_matrix_part(self, start_row=0, num_rows=-1, start_col=0,
        ...
        if not self.matrices_ready_flag:
            if self.params_set_flag:
                ...
                print(message)
                return -1, np.zeros((1, 1))
        if (start_row + num_rows) > self.ofmap_px_per_filt * self.batch_size or (start_col + num_cols) > self.num_filters:
            message = err_prefix + ": Illegal arguments. Exiting!"
            print(message)
            return -2, np.zeros((1, 1))
