...
class ReadBufferEstimateBw:
    ...
    def complete_all_prefetches(self):
        ...
        if not self.active_buffer_prefetch_done:
            ...
            cycles_needed = (num_sets_to_prefetch * self.num_items_per_set) \
                            / self.prefetch_bandwidth
            #print('CC:{}   flag:{}'.format(current_set_elems, self.active_buffer_prefetch_done)) ###
            cycles_needed = math.ceil(cycles_needed)

            self.last_prefetch_start_cycle = self.last_prefetch_end_cycle - cycles_needed + 1
            #print('.last_prefetch_start_cycle:{}   end_cycle:{}   cycles_needed:{}'.format(self.last_prefetch_start_cycle,self.last_prefetch_end_cycle,cycles_needed)) ###
    ...
    def prefetch(self):
        ...
        cycles_needed = self.last_prefetch_end_cycle - self.last_prefetch_start_cycle + 1
        #print('cycles_needed: {}'.format(cycles_needed)) ################################
        ...
        #print('prefetch_requests: {}'.format(prefetch_requests.shape)) ################################
        ...
        #print('this_prefetch_traces: {}'.format(this_prefetch_traces.shape)) ################################
