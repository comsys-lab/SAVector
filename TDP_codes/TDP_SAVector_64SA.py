...
if __name__ == '__main__':
    ...
    Arr_row = int(SArow)
    Arr_col = int(SAcol)
    IF_SRAM = int(1048576) #1MB
    Fil_SRAM = int(1048576) #1MB
    OF_SRAM = 524288
    mkcfg(cfgname, dflow, Arr_row, Arr_col, IF_SRAM, Fil_SRAM, OF_SRAM)

    grid_arr_dict = {'2x2':cfgfname, '2x4':cfgfname, '4x2':cfgfname, '4x4':cfgfname, '4x8':cfgfname, '8x4':cfgfname, '8x8':cfgfname, '8x16':cfgfname, '16x8':cfgfname,}
    #############################
    ...
    for gds in x_labels:
        ...
        SRAMS=[]
        RT=[]
        repname = "SAVector_" + "64x64SA_" + str(gds) + 'p'
        repdir='./TDP/SAVector_detail/'+toponame #./reports_detail/simname_toponame
        createDirectory('./SAVector_detail')
        ...
        for i in range(number_layers):
            ...
            for j in range(g_row): # j=0,1,...,grid_row-1
                if_temp = if_temp + ifmap_dram_pod[i,(j*g_row)]
        ...
        repdir2='./TDP/SAVector_Compact/'+toponame #./reports_compact/simname_toponame
        createDirectory('./TDP/SAVector_Compact')
