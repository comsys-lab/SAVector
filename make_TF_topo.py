...
topotype=input('topo type (TF, BERT, ViT, GPT): ')
vocab=int(input('vocabulary size (seq len): '))
embed=int(input('embedding size (d_model): '))
head=int(input('number of heads: '))
assert embed % head == 0
dk=int(embed/head)

fname='./topologies/TFs_mnk/'+toponame+'.csv'
...
if topotype=='TF':
    ...
    wr.writerow(['FF1',vocab,embed,4*embed,''])
    wr.writerow(['FF2',vocab,4*embed,embed,''])
elif (topotype=='BERT') or (topotype=='ViT'):
    #encoder
    wr.writerow(['Layer','M','N','K',''])
    for i in range(head):
